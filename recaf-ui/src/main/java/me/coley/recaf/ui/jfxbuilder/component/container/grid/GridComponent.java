...
import me.coley.recaf.ui.jfxbuilder.NodeToggle;
import me.coley.recaf.ui.jfxbuilder.component.container.stage.BuildStage;
...
public class GridComponent implements IGridComponent {
	...
	public GridComponent add(Node node) {
		switch (strategy) {
			case ROW:
				if (node instanceof NodeToggle) {
					node.styleProperty().bind(((NodeToggle) node).visibilityProperty().map(visible -> "-fx-border-width: " + (visible ? vgap : 0) + ",0,0,0; -fx-border-color: transparent"));
				} else {
					node = new Pane(node);
					node.setStyle("-fx-border-width: " + vgap + ",0,0,0; -fx-border-color: transparent");
				}
				...
				if (nodes.length != columns) {
					...
					for (Node node : nodes) {
						if (node == null) column++;
							...
							if (node instanceof NodeToggle) {
								node.styleProperty().bind(((NodeToggle) node).visibilityProperty().
										map(visible -> "-fx-border-width: " + (visible ? vgap : 0) + ",0,0,0; -fx-border-color: transparent"));
							} else {
								node = new Pane(node);
								node.setStyle("-fx-border-width: " + vgap + ",0,0,0; -fx-border-color: transparent");
							}
							grid.add(node, column++, row, 1, 1);
						}
					}
					row++;
				}
	...
	public IGridComponent addIf(ObservableBooleanValue condition, Node node) {
		return add((Node) NodeToggle.nodeToggle(condition, node));
	}

	@Override
	public IGridComponent addIf(ObservableBooleanValue condition, Node... nodes) {
		return add(Arrays.stream(nodes).map(node -> node == null ? null : (Node) NodeToggle.nodeToggle(condition, node)).toArray(Node[]::new));
	}
}
