...
/**
 * Common base for decompiling {@link CommonClassInfo}.
 *
 * @author Matt Coley
 * @see DexDecompilePane Implementation for android.
 * @see DecompilePane Standard implementation.
 */
public abstract class CommonDecompilePane extends BorderPane implements ClassRepresentation, Cleanable, Scrollable {
	private static final Logger log = Logging.get(DexDecompilePane.class);
	private final ClearableThreadPool threadPool = new ClearableThreadPool(1, true, "Decompile");
	protected final JavaArea javaArea;
	protected CommonClassInfo lastClass;
	private boolean ignoreNextDecompile;

	public CommonDecompilePane() {
	...
	public void onUpdate(CommonClassInfo newValue) {
		...
		if (!checkDecompilePreconditions()) {
		...
		CompletableFuture<String> decompileFuture = CompletableFuture.supplyAsync(supplyDecompile(), threadPool)
	...
	protected abstract Supplier<String> supplyDecompile();

	protected abstract boolean checkDecompilePreconditions();
