...
public class ElfExplorerPane extends SplitPane implements FileRepresentation {
    ...
    private final StringTableTableDisplayMode STRING_TABLE_MODE = new StringTableDisplayMode();
    ...
    public void onUpdate(FileInfo newValue) {
        ...
        try {
            ...
            logger.error("Failed to parse ELF file: %s", e.getMessage());
        }
     ...
     *
     * @param observable Wrapper.
     * @param oldValue   Old selected item.
    ...
    private void onSelectionChange(ObservableValue<? extends TreeItem<String>> observable,
        ...
        if (newValue == itemElfHeader) {
            ...
            STRING_TABLE_MODE.apply(elfFile, elfFile.getStringTable(), primaryTableView);
        } else if (newValue == itemDynamicStringTable) {
            STRING_TABLE_MODE.apply(elfFile, elfFile.getDynamicStringTable(), primaryTableView);
        } else {
            ...
            if (programHeaderIndex != -1) {
                ...
                PROGRAM_HEADER_MODE.apply(elfFile, programHeader, primaryTableView);
            } else if (sectionHeaderIndex != -1) {
                ElfSectionHeader sectionHeader = elfFile.getSection(sectionHeaderIndex).header;
                SECTION_HEADER_MODE.apply(elfFile, sectionHeader, primaryTableView);
            } else {
                primaryTableView.getColumns().clear();
            }
        }
