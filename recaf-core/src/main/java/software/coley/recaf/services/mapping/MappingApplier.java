...
import me.darknet.dex.tree.remap.ClassDefinitionRemapper;
import me.darknet.dex.tree.remap.DefinitionRemapper;
...
public class MappingApplier {
	...
	public MappingResults applyToClasses(@Nonnull Mappings mappings,
	                                     ...
	                                     @Nonnull Collection<AndroidClassInfo> classes) {
		mappings = enrich(mappings);
		MappingApplicationListener listener = listeners == null ? null : listeners.createBundledMappingApplicationListener();
		MappingResults results = new MappingResults(workspace, mappings, listener);
		if (aggregateMappingManager != null)
		...
		// Apply mappings to the provided classes, collecting into the results model.
		Mappings finalMappings = mappings;
		ClassDefinitionRemapper remapper = newAndroidRemapper(finalMappings);
		ExecutorService service = ThreadUtil.phasingService(applierThreadPool);
		for (AndroidClassInfo classInfo : classes)
			service.execute(() -> dumpIntoResults(results, workspace, resource, bundle, classInfo, remapper));
		ThreadUtil.blockUntilComplete(service);
	...
	public MappingResults applyToPrimaryResource(@Nonnull Mappings mappings) {
		...
		resource.androidClassBundleStreamRecursive().forEach(bundle -> {
			bundle.forEach(classInfo -> {
				service.execute(() -> dumpIntoResults(results, workspace, resource, bundle, classInfo, remapper));
			});
		});
	 ...
	 * 		The class to apply mappings to.
	 * @param remapper
	 * 		The remapper backed by the mappings to apply.
	 */
	private static void dumpIntoResults(@Nonnull MappingResults results,
	                                    @Nonnull Workspace workspace,
	                                    @Nonnull WorkspaceResource resource,
	                                    ...
	                                    @Nonnull ClassDefinitionRemapper remapper) {
		String originalName = classInfo.getName();

		// Apply renamer
		ClassDefinition original = classInfo.getBackingDefinition();
		ClassDefinition mapped = remapper.remap(original);

		// Update class if it has any modified references
		if (!original.equals(mapped)) {
			AndroidClassInfo updatedInfo = new AndroidClassInfoBuilder(mapped).build();
	...
	@Nonnull
	private static ClassDefinitionRemapper newAndroidRemapper(@Nonnull Mappings mappings) {
		return new ClassDefinitionRemapper(new DefinitionRemapper() {
			@Nonnull
			@Override
			public String mapInternalName(@Nonnull String internalName) {
				String mappedClassName = mappings.getMappedClassName(internalName);
				return Objects.requireNonNullElse(mappedClassName, internalName);
			}

			@Nonnull
			@Override
			public String mapFieldName(@Nonnull InstanceType owner, @Nonnull String name, @Nonnull ClassType type) {
				String mappedFieldName = mappings.getMappedFieldName(owner.internalName(), name, type.descriptor());
				return Objects.requireNonNullElse(mappedFieldName, name);
			}

			@Nonnull
			@Override
			public String mapMethodName(@Nonnull ReferenceType owner, @Nonnull String name, @Nonnull MethodType type) {
				String mappedMethodName = mappings.getMappedMethodName(owner.internalName(), name, type.descriptor());
				return Objects.requireNonNullElse(mappedMethodName, name);
			}
		});
	}
}
