...
import me.coley.recaf.util.visitor.AnnotationArrayVisitor;
...
public class KotlinMetadataVisitor extends AnnotationVisitor {
	...
	public AnnotationVisitor visitArray(String name) {
		...
		if (DATA1_NAME.equals(name)) {
			visitor = new AnnotationArrayVisitor<>(visitor, (List<String> contents) -> {
				data1 = contents.toArray(new String[0]);
			});
		} else if (DATA2_NAME.equals(name)) {
			visitor = new AnnotationArrayVisitor<>(visitor, (List<String> contents) -> {
				data2 = contents.toArray(new String[0]);
			});
		} else if (METADATA_VERSION_NAME.equals(name)) {
			visitor = new AnnotationArrayVisitor<>(visitor, (List<Integer> contents) -> {
				metadataVersion = contents.stream().mapToInt(i -> i).toArray();
			});
		} else if (BYTECODE_VERSION_NAME.equals(name)) {
			visitor = new AnnotationArrayVisitor<>(visitor, (List<Integer> contents) -> {
				bytecodeVersion = contents.stream().mapToInt(i -> i).toArray();
			});
		}
		return visitor;
	}
