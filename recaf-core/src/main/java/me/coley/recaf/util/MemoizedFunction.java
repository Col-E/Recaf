...
public class MemoizedFunction {
	private static class UniMemoizedFunction<Key, Value> implements Function<Key, Value> {
		private final Map<Key, Value> cache = new HashMap<>();
		private final Function<Key, Value> function;

		private UniMemoizedFunction(Function<Key, Value> function) {
	...
	public static <Key, Value> Function<Key, Value> memoize(Function<Key, Value> function) {
		return new UniMemoizedFunction<>(function);
	}
