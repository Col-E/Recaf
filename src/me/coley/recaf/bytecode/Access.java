...
public class Access {
	...
	private static final Map<String, Integer> accessMap = new HashMap<String, Integer>() {{
		...
		put("VOLATILE",     VOLATILE    ); // same value as BRIDGE
		put("TRANSIENT",    TRANSIENT   );
		put("NATIVE",       NATIVE      );
		put("INTERFACE",    INTERFACE   );
		put("ABSTRACT",     ABSTRACT    );
		put("STRICT",       STRICT      );
		put("BRIDGE",       BRIDGE      ); // same value as VOLATILE
		put("VARARGS",      VARARGS     );
	...
	@SuppressWarnings("serial")
	private static final Map<Integer, String> accessReverseMap = new HashMap<Integer, String>() {{
		put(PUBLIC,       "PUBLIC"      );
		put(PRIVATE,      "PRIVATE"     );
		put(PROTECTED,    "PROTECTED"   );
		put(STATIC,       "STATIC"      );
	...
	// value <--> name
	public static int   value(String name) { return accessMap.get(name); }
	public static String name(int access)  { return accessReverseMap.get(access); }
	// Access checking
