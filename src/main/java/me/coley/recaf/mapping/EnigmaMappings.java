		...
		for(String lineStr : lines) {
			...
			try {
				...
				case "CLASS":
					...
					// Not all classes need to be renamed if they have child elements that are
					// renamed
					if (args.length >= 3) {
				...
				case "METHOD":
					...
					if (currentMethod.equals("<init>"))
						continue;
					// Not all methods need to be renamed if they have child arg elements that are
					// renamed
					if (args.length >= 4) {
						...
						workspace.getHierarchyGraph().getHierarchyNames(currentClass.peek())
								.forEach(hierarchyMember -> map.put(hierarchyMember + "." + currentMethod + methodType,
										renamedMethod));
					}
					break;
				case "ARG":
